...
class RegionProposalNetwork(nn.Module):
    def __init__(self, in_channels=512, mid_channels=512,
                 ratios=[0.5, 1, 2], anchor_scales=[8, 16, 32], feat_stride=16, mode="training",):
    ...
    def forward(self, x, img_size, scale=1.):
        ...
        rpn_locs = self.loc(x)
        """
        利用permute()将[B、C、H、W]换为[B、H、W、C]
        在torch里面，view()相当于numpy的reshape,但是如果在view()之前用了permute()等函数，
        ...
        # 生成先验框，此时获得的anchor是布满网格点的，当输入图片为600,600,3的时候，shape为(38*38*9, 4)
